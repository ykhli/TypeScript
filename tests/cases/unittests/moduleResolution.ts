...
module ts {
    ...
    const loadOnlyModulesImportResolutionContext: ImportResolutionContext = { loadOnlyTrueExternalModules: true };
    describe("Node module resolution - relative paths", () => {
        ...
        function testLoadAsFile(containingFileName: string, moduleFileNameNoExt: string, moduleName: string): void {
            ...
            for (let ext of supportedExtensions) {
                ...
                let resolution = nodeModuleNameResolver(moduleName, containingFile.name, defaultImportResolutionContext, createModuleResolutionHost(containingFile, moduleFile));
                assert.equal(resolution.resolvedModule.resolvedFileName, moduleFile.name);
                assert.equal(resolution.resolvedModule.shouldBeTrueExternalModule, false);
            ...
            for (let ext of supportedExtensions) {
                let containingFile = { name: containingFileName }
                let moduleFile = { name: moduleFileNameNoExt + ext }
                let resolution = nodeModuleNameResolver(moduleName, containingFile.name, loadOnlyModulesImportResolutionContext, createModuleResolutionHost(containingFile, moduleFile));
                if (ext === ".d.ts") {
                    assert.equal(resolution.resolvedModule.resolvedFileName, moduleFile.name);
                    assert.equal(resolution.resolvedModule.shouldBeTrueExternalModule, true);
                    assert.equal(resolution.failedLookupLocations.length, 0);
                }
                else {
                    assert.equal(resolution.resolvedModule, undefined, "should load only .d.ts files");
                }
            }
        }
        ...
        function testLoadingFromPackageJson(containingFileName: string, packageJsonFileName: string, fieldRef: string, moduleFileName: string, moduleName: string): void {
            ...
            let resolution = nodeModuleNameResolver(moduleName, containingFile.name, defaultImportResolutionContext, createModuleResolutionHost(containingFile, packageJson, moduleFile));
            assert.equal(resolution.resolvedModule.resolvedFileName, moduleFile.name);
            assert.equal(resolution.resolvedModule.shouldBeTrueExternalModule, false);
        ...
        it ("module name as directory - load index.d.ts", () => {
            ...
            let resolution = nodeModuleNameResolver("./foo", containingFile.name, defaultImportResolutionContext, createModuleResolutionHost(containingFile, packageJson, indexFile));
    ...
    describe("Node module resolution - non-relative paths", () => {
        it("load module as file - ts files not loaded", () => {
            ...
            let resolution = nodeModuleNameResolver("foo", containingFile.name, defaultImportResolutionContext, createModuleResolutionHost(containingFile, moduleFile));
        ...
        it("load module as file", () => {
            ...
            let resolution = nodeModuleNameResolver("foo", containingFile.name, defaultImportResolutionContext, createModuleResolutionHost(containingFile, moduleFile));
            assert.equal(resolution.resolvedModule.resolvedFileName, moduleFile.name);
            assert.equal(resolution.resolvedModule.shouldBeTrueExternalModule, true, "module with non-relative name is always loaded as true external module");
        });
