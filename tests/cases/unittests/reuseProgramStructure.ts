            ...
            for (let id in expectedContent) {
                if (hasProperty(expectedContent, id)) {
                    ...
                    if (expectedContent[id]) {
                        assert.isTrue(file.resolvedModules[id] !== undefined,`module ${id} should be resolved`);
                        assert.equal(file.resolvedModules[id].resolvedFileName, expectedContent[id].resolvedFileName, `resolvedFileName should be equal, expected ${file.resolvedModules[id]}, actual${expectedContent[id]}`);
                        assert.equal(!!file.resolvedModules[id].shouldBeTrueExternalModule, !!expectedContent[id].shouldBeTrueExternalModule, `shouldBeTrueExternalModule should be equal, expected ${JSON.stringify(file.resolvedModules[id])}, actual${JSON.stringify(expectedContent[id])}`);
                    }
                    else {
                        assert.isTrue(file.resolvedModules[id] === undefined,`module ${id} should not be resolved`);
                    }
                }
            }
        }
